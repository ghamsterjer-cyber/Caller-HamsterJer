...
export default function WebRTCCaller() {
  ...
  const cleanup = useCallback(
    async (isInitiatedByRoomDeletion = false) => {
      ...
      toast({
        ...
        description: 'Вы можете начать новый вызов.',
      });
    },
      ...
      try {
        ...
        pc.onicecandidate = event => {
          if (event.candidate && roomKey) {
            ...
            const candidatesRefPath = `rooms/${roomKey}/${
        ...
        pc.onconnectionstatechange = () => {
          ...
          switch (pc.connectionState) {
            ...
            case 'failed':
              if (
                ['active', 'waiting', 'joining', 'creating'].includes(
                  callStateRef.current
                )
              ) {
                addLog(
                  `Соединение разорвано (состояние: ${pc.connectionState}). Завершение вызова.`
                );
  ...
  const handleCreateCall = async () => {
    ...
    setupFirebaseListener(`rooms/${roomKey}`, snapshot => {
      ...
      if (roomData.answer && !isAnswerSetRef.current) {
        ...
        pc.setRemoteDescription(new RTCSessionDescription(roomData.answer)).catch(
          e => {
    ...
    onValue(ref(database, calleeCandidatesRefPath), snapshot => {
      snapshot.forEach(childSnapshot => {
        ...
        pc.addIceCandidate(new RTCIceCandidate(childSnapshot.val())).catch(e =>
          addLog(`Ошибка при добавлении ICE кандидата: ${e.message}`)
        );
      });
    });
    firebaseListenersRef.current.push({
      path: calleeCandidatesRefPath,
      type: 'value',
    });
      ...
      if (roomData.offer && !pc.remoteDescription) {
        ...
        (async () => {
          try {
            ...
            const message = e instanceof Error ? e.message : String(e);
            addLog(
              `Ошибка при обработке Offer или создании Answer: ${message}`
            );
            setCallState('failed');
          }
        })();
    ...
    switch (callState) {
      case 'idle':
        return {
          ...
          desc: 'Создайте комнату или присоединитесь к существующей.',
        };
      case 'creating':
      case 'joining':
        return {
          ...
          title: 'Подключение...',
          desc: 'Инициализация вызова, пожалуйста, подождите.',
        };
      case 'waiting':
        return {
          icon: <Loader2 className="animate-spin" />,
          title: 'Ожидание собеседника...',
          desc: `Поделитесь ключом комнаты: ${roomKey}`,
        };
      case 'active':
        return {
          icon: <Phone className="text-green-500 animate-pulse" />,
          title: 'Вызов активен',
          desc: 'Вы в эфире. Говорите.',
        };
      case 'failed':
        return {
          icon: <PhoneOff className="text-red-500" />,
          title: 'Сбой соединения',
          desc: 'Проверьте подключение или попробуйте еще раз.',
        };
      case 'ending':
        return {
          icon: <PhoneOff />,
          title: 'Завершение...',
          desc: 'Завершение вызова и очистка ресурсов.',
        };
      default:
      ...
      <div className="w-full max-w-2xl mx-auto space-y-4">
        <Card className="w-full shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              P2P-звонки через интернет с помощью WebRTC
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="roomKey" className="font-medium">
                Ключ комнаты
              </label>
              <div className="flex gap-2">
                ...
                />
                <Button
                  id="tutorial-generate-key-button"
                  variant="outline"
                  size="icon"
          ...
          <CardFooter className="flex flex-col sm:flex-row gap-2">
            {callState === 'idle' && (
              <>
                <Button
                  id="tutorial-create-room-button"
              ...
              <ScrollArea className="h-48 w-full rounded-md border p-4 font-mono text-xs">
                ...
                {logs.length === 0 && (
                  <div className="text-muted-foreground">
                    Здесь будут отображаться логи событий...
                  </div>
                )}
              </ScrollArea>
            </CardContent>
